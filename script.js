...
function cargarProductos() {
    fetch('productos.json')
        ...
        .then(data => {
            // Asegurar que todos los productos tengan el campo disponible y categoría como array
            productos = data.map(producto => ({
                ...
                contraindicaciones: producto.contraindicaciones || "No hay contraindicaciones específicas.",
                // Asegurar que categoría sea un array
                categoria: Array.isArray(producto.categoria) ? producto.categoria : [producto.categoria]
            }));
            renderizarProductos();
        })
        .catch(error => {
...
function renderizarProductos() {
    ...
    const productosFiltrados = currentCategory === 'todos' 
        ...
        : productos.filter(p => p.disponible && p.categoria.includes(currentCategory));
...
function inicializarCarrusel(promociones) {
    ...
    promociones.forEach((promo, index) => {
        ...
        const backgroundStyle = promo.background && promo.fondo === 'imagen' 
